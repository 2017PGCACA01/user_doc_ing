...
def test_ingestion_success(mock_post, app, db, tmp_path):
    ...
    mock_response.json.return_value = {"message": {"content": "mocked summary"}}
...
def test_ingestion_file_not_found(app, db, tmp_path):
    with app.app_context():
        ...
        db.session.add(doc)
        db.session.commit()
        doc_id = doc.id  # ✅ capture before context ends
    ...
    with app.app_context():
        ing = Ingestion.query.filter_by(document_id=doc_id).first()
        assert ing.status == "failed"
        assert "No such file" in ing.error_message or "No such file or directory" in ing.error_message
